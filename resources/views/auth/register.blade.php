    ...
    <div class="flex flex-col items-center justify-center min-h-[80vh]">
        <div class="bg-stone-900/90 shadow-xl rounded-2xl px-8 py-10 w-full max-w-md border border-stone-800">
            ...
            <form method="POST" action="{{ route('register') }}" class="space-y-6">
                ...
                <div>
                    <x-primary-button
                        class="w-full bg-amber-400 hover:bg-amber-500 text-stone-900 font-bold py-2 rounded-lg transition">{{ __('Register') }}</x-primary-button>
                </div>
                <div class="text-center mt-4">
                    <a href="{{ route('login') }}"
                        class="text-sm text-stone-400 hover:text-amber-400 transition">Already registered? Log in</a>
                </div>
            </form>
        </div>
    </div>
